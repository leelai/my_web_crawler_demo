...
baseUrl = 'https://reg.ntch.ntpc.gov.tw:4443'
openUrl = baseUrl + '/OPDREGWEB/'
queryUrl = baseUrl + '/OPDREGWEB/Query/RegQry_1.aspx'
...
def checking(name, id, birth):
  ...
  try:
    ...
    for tr in trs:
      if tr.text.find('看診時間') >= 0:
        continue
      store_details = {"姓名":name, "身份證": id, "日期":None, "科別診室":None, "診號": None, '地點': None, "候診參考時間": None, "醫院": '新北市立聯合醫院'}
      tds = tr.find_all('td')
      # print(tds)
      store_details['日期'] = tds[0].text.strip().replace(' ', '') + tds[1].text.strip().replace(' ', '')
      # pattern = re.compile(r'\s+')
      # sentence = re.sub(pattern, '', tds[2].text)
      store_details['科別診室'] = tds[2].text.strip().replace(' ', '') + tds[3].text.strip().replace(' ', '')
      store_details['診號'] = tds[5].text.strip().replace(' ', '')
      store_details['候診參考時間'] = tds[0].text.strip().replace(' ', '') + tds[1].text.strip().replace(' ', '')
      store_details['地點'] = tds[2].text.strip().replace(' ', '')
      store_list.append(store_details)
  except Exception as e:
    print('http error (' + name + ' ' + id + ' 新北市立聯合醫院 )' + str(e))
  # print(store_list)
  return store_list
